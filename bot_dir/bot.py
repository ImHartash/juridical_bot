...
async def start_handler(msg: Message):
    ...
    await msg.answer_photo(
        photo=FSInputFile(
            r"C:\Users\user\PycharmProjects\pythonProject\juridical_bot\txts\main_pic.jpg"
        ),
        caption=start_text,
...
async def feedback_handler(msg: Message, state: FSMContext):
    ...
    if FeedbackDatabase().is_user_send_feedback(user_id=user_id):
        ...
        await msg.answer(
            text=f"Ваш последний отзыв: \n {feedback}. \n Желаете его изменить?",
            reply_markup=keyboards.changekb,
        )
        return
    else:
...
async def start_ai(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer(text=start_ai_text)
    if MessagesDatabase().count_all_users(user_id) > MAX_USERS:
        await cb.message.answer(text="Превышено кол-во пользователей. Пока.")
        return
...
async def docs_callback(cb: CallbackQuery):
    ...
    await cb.message.answer(
        text=example_docs_text, reply_markup=keyboards.examples_keyboard
    )
...
async def ai_answer(msg: Message, state: FSMContext):
    ...
    if "продолжи" in message.lower():
        ...
        assistant_content = MessagesDatabase().last_row(
            user_id=user_id, role="assistant"
        )[0][0]
        status, response, tokens_in_response = continue_gpt_response(
    ...
    if message.lower() != "закончить ответ" and message.lower() != "продолжи ответ":
        MessagesDatabase().insert_row(
            user_id=user_id, message=message, role="user", tokens=count_tokens(message)
        )
